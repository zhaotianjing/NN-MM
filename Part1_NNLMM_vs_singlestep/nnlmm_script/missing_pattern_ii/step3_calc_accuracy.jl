...
Random.seed!(123)
method="omics_gblup"
main_path="/group/qtlchenggrp/tianjing/omics_fix_all/random_missing/"
    ...
    for i in 1:nRep
        ...
        if ispath("rep$i/results/MCMC_samples_EBV_NonLinear.txt")
            ...
            if size(mcmc,1)==1000
                ...
                #plot results
                myfig=plot(collect(chain_length/1000:chain_length/1000:chain_length),accuracy_datai,xlabel="iteration",ylabel="accuracy",title="random missing.pct$pct.rep$i.accu=$final_accuracy")
